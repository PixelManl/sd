...
XML_DIR = 'defect_xml'
OUT_DIR = 'train_data_closeup/10_missing_fastener'
CLOSEUP_SCALE = 3.0
OUT_SIZE = 512
IMG_EXTS = ['.jpg', '.jpeg', '.png', '.bmp', '.webp']
CAPTION = 'missing nut defect, power grid equipment, metal fastener, close-up'
...
def square_crop(img, box, scale=3.0, out_size=512):
    h, w = img.shape[:2]
    _, x1, y1, x2, y2 = box
    bw = max(1, x2 - x1)
    bh = max(1, y2 - y1)
...
def main():
    ...
    for xml_path in xml_files:
        ...
        for i, box in enumerate(objs):
            crop = square_crop(img, box, scale=CLOSEUP_SCALE, out_size=OUT_SIZE)
            base = f'{stem}_{i:02d}_closeup'
            cv2.imwrite(str(Path(OUT_DIR) / f'{base}.png'), crop)
            with open(Path(OUT_DIR) / f'{base}.txt', 'w', encoding='utf-8') as f:
                f.write(CAPTION)
            count += 1
    print(f'[DONE] close-up ROI samples: {count}')
